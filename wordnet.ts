    ...
    for (const word of WordNet) {
        ...
        words.forEach(definition => {
            ...
            definition.meta?.pointers.forEach(pointer => {
                ...
                //     let data: Map<string, any> = dictionary.has(pointer.pos) ? dictionary.get(pointer.pos)! : dictionary.set(pointer.pos, new Map())
                //     data = data.has(word) ? data.get(word)! : data.set(word, new Map());
                //     data.set(word, pointerWord)
                // });
