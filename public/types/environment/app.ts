...
import {MultiGraph} from "graphology";import * as Openpgp from "openpgp";
...
(async () => {
  ...
  //   // const graphData = graph.graphData();
  //   const extendedKey = (document.querySelector("#key") as any).value;
...
async function onActivate() {
  ...
  try {
    (document.querySelector("#connect") as any).addEventListener("click", onConnet);

    (document.querySelector("#create") as any).addEventListener("click", onCreate);

    (document.querySelector("#register") as any).addEventListener("click", onRegister);

    (document.querySelector("#activate") as any).addEventListener("click", onActivate);
  } catch (error) {
    console.warn(error)
  }
    ...
    try {
      ...
      switch (type) {
        ...
        case 'DATA_ENCRYPTED':
          ...
          break;
        case 'EDIT':
          (document.getElementById('edit-dialog') as any).showModal()
          break;
        case 'GRAPH_2D':
          (document.getElementById('graph2d-dialog') as any).showModal()
          break;
        case 'WALLET':
          (document.getElementById('wallet-dialog') as any).showModal()
          break;
        case 'LOGIN':
          (document.getElementById('login-dialog') as any).showModal()
          break;
        case 'SIGN':
          (document.getElementById('sign-dialog') as any).showModal()
          break;
        case 'BLOCK':
          (document.getElementById('block-dialog') as any).showModal()
          break;
        case 'TRANSACTION':
          (document.getElementById('transaction-dialog') as any).showModal()
          break;
      }
    } catch (error) {
      console.debug(error);
  ...
  async function onVerifyProof() {
    const proofInput = (document.getElementById('merkleProofInput') as any).value;
    const proof = JSON.parse(proofInput || '[]');
    const tree = new MerkleTree(['a', 'b', 'c'], ethers.sha256);
    const root = tree.getRoot().toString('hex');
  ...
  try {
    (document.getElementById("get-qrcode") as any).addEventListener("click", onQRCode);
    // Ethers.js Example: Create Wallet
    (document.getElementById('createWalletBtn') as any).addEventListener('click', onCreateWallet);
    // Ethers.js Example: Sign Message
    (document.getElementById('signMessageBtn') as any).addEventListener('click', onSignMessage);
    // Ethers.js Example: Verify Message
    (document.getElementById('verifyMessageBtn') as any).addEventListener('click', onVerifyMessage);
    // MerkleTree.js Example: Verify Proof
    (document.getElementById('verifyProofBtn') as any).addEventListener('click', onVerifyProof);
  } catch (error) {
    console.warn(error)
  }
})();
function handleSignedTransaction(signedTransaction: any) {
  throw new Error("Function not implemented.");
}

function handleEncryptedData(encryptedData: any) {
  throw new Error("Function not implemented.");
}
