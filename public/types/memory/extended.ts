...
import { Memory, BaseMemory } from "./index.";

function getKey(id: string): Uint8Array {
...
export class MemoryExtension extends Memory implements IMemoryExtension {
    ...
    async exportBlocks(hashes: string[]): Promise<{ blocks: iBlock[], merkleRoot: string }> {
        ...
        for (const hash of hashes) {
            ...
            if (block) {
                blocks.push(block);
                leaves.push(sha256(JSON.stringify(block)).toString()); // Hash each block
            }
        }
    ...
    async importBlocks(blocks: iBlock[], merkleRoot: string): Promise<void> {
        const leaves = blocks.map(block => sha256(JSON.stringify(block)).toString());
