...
expect.extend({
    toBeValidCueEvent(received) {
        ...
        return {
            pass,
            message: () => `${received} ${isNot ? 'is' : 'is not'} a valid CUE event`
        };
    },
    toHaveValidSignature(received) {
        const { isNot } = this;
        const pass = received &&
        ...
        return {
            pass,
            message: () => `${received} ${isNot ? 'has' : 'does not have'} a valid cryptographic signature`
        };
    },
    toPassAxiomaticValidation(received) {
        const { isNot } = this;
        // Mock axiomatic validation for testing
        const pass = received && received.harmonicSignature && received.phase !== undefined;
        return {
            pass,
            message: () => `${received} ${isNot ? 'passes' : 'fails'} axiomatic validation`
        };
    }
});
...
export const CueTestUtils = {
    ...
    createMockSignedMessage: (payload, overrides = {}) => ({
        payload,
    ...
    generateMockTemperatureReadings: (count, baseTemp = 22.0) => {
