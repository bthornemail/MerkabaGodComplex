...
const App: React.FC = () => {
  const [quadrantInputs, setQuadrantInputs] = useState<QuadrantState[]>([
    { id: 1, type: 'text', text: 'Freedom: The inherent power to choose and act without external coercion.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Freedom' },
    { id: 2, type: 'text', text: 'Autonomy: Self-governance and independent decision-making.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Autonomy' },
    { id: 3, type: 'text', text: 'Reciprocity: Mutual exchange of benefits or privileges.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Reciprocity' },
    { id: 4, type: 'text', text: 'Sovereignty: Supreme authority and self-determination within a domain.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Sovereignty' },
    ...
    { id: 12, type: 'text', text: 'Publish: Initiate data broadcast to interested parties.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Publish' },
    { id: 13, type: 'text', text: 'Subscribe: Register interest in receiving data from a specific source or topic.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Subscribe' },
    { id: 14, type: 'text', text: 'Topic: A channel or category for data exchange.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Topic' },
    { id: 15, type: 'text', text: 'Message: The discrete unit of information transmitted.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Message' },
    // New quadrants for User Transaction Space
    { id: 16, type: 'text', text: 'Actor: The user or agent performing the action.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Actor' },
    { id: 17, type: 'text', text: 'Action: The specific interaction or operation (e.g., "create_doc", "transfer_asset").', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Action' },
    { id: 18, type: 'text', text: 'State: The resulting condition or outcome after the action is processed.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'State' },
    { id: 19, type: 'text', text: 'Timestamp: The cryptographic time of the transaction, for ordering and verification.', file: null, fileType: null, buffer: null, signature: null, fixedDimensionRay: null, ubhpType: 'Timestamp' },
  ]);
  ...
  const [universalModelHarmony28D, setUniversalModelHarmony28D] = useState<number[] | null>(null); // New
  ...
  const USER_TRANSACTION_MASTER_SEED = "UBHP_User_Transaction_Master_Key_For_Interaction_Space"; // New

  // Canvas state
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [canvasCtx, setCanvasCtx] = useState<CanvasRenderingContext2D | null>(null);
  const [selectedSymmetryType, setSelectedSymmetryType] = useState<string>('all_inputs'); // Default to all inputs
  const [allHarmonicVectors, setAllHarmonicVectors] = useState<Map<number | string, HarmonicVector>>(new Map()); // Store all generated HVs
  ...
  // Handler for deriving the 28D Universal Model Harmony (Final)
  const handleDerive28DUniversalModelHarmony = () => {
    ...
    // Concatenate the 14D, Discreet Layer 7D, and User Transaction 7D vectors to form a 28D vector
  ...
  const drawPoints = useCallback(() => {
    ...
    if (selectedSymmetryType === 'all_inputs') {
      allHarmonicVectors.forEach((hv, key) => {
        if (typeof key === 'number') { // Only quadrant inputs (1-19)
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 font-sans p-4 sm:p-8 flex flex-col items-center">
      ...
      <h1 className="text-4xl sm:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 mb-8 text-center">
        UBHP 28D Universal Model Harmony
      </h1>
      <p className="text-gray-400 text-lg mb-8 text-center max-w-3xl">
        ...
        to a globally contextualized, operationally defined, and user-interaction-aware 28-dimensional representation.
      </p>
      ...
      <h2 className="text-3xl font-semibold mt-12 mb-6 text-teal-400">User Transaction Space (5D Rays)</h2>
      <p className="text-gray-400 text-lg mb-8 text-center max-w-3xl">
        This layer represents the pure user transaction space, forming a component of the hypergraph's
        binary vector clock for user interactions.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-7xl mb-8">
      ...
      <div className="mt-8 w-full max-w-4xl bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-center">
        ...
        <p className="text-gray-400 text-sm mb-4">
          This centroid represents the common harmony of the Actor, Action, State, and Timestamp components,
      ...
      {/* 28D Universal Model Harmony Derivation - NEW */}
      <div className="mt-8 w-full max-w-4xl bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-center">
        <h2 className="text-2xl font-semibold mb-4 text-purple-300">28D Universal Model Harmony (Hypergraph Binary Vector Clock)</h2>
        <p className="text-gray-400 text-sm mb-4">
          This 28D vector concatenates the 14D Universal Addressable Model Harmony Feature Set,
          the 7D Discreet Model Layer Vector, and the 7D User Transaction Vector. It represents
          the full, multi-dimensional addressable identity and state of a user interaction within
          the universal hypergraph model.
        </p>
        <button
          onClick={handleDerive28DUniversalModelHarmony}
          className="w-full bg-gradient-to-r from-purple-700 to-indigo-700 hover:from-purple-800 hover:to-indigo-800 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-600"
        >
          Derive 28D Universal Model Harmony
        </button>

        {universalModelHarmony28D !== null && (
          <div className="mt-6 p-4 bg-gray-700 rounded-lg border border-gray-600 text-left">
            <h3 className="text-lg font-medium text-pink-300 mb-2">Derived 28D Universal Model Harmony Vector:</h3>
            <p className="break-all text-sm font-mono text-gray-300">
              [{universalModelHarmony28D.map(val => val.toFixed(6)).join(', ')}]
            </p>
            <p className="text-gray-400 text-xs mt-2">
              This 28D vector embodies the full scope of the demonstrated UBHP dimensional expansion,
              integrating foundational principles, global semantic context, discrete operational layers,
              and user transaction space into a unified, addressable geometric representation,
              acting as a binary vector clock for hypergraph interactions.
            </p>
          </div>
        )}
      </div>
        ...
        {universalModelHarmony28D !== null && (
          <div className="p-4 bg-gray-700 rounded-lg border border-gray-600 text-left">
            <h3 className="text-xl font-semibold text-red-300 mb-2">
              The 28D Universal Model Harmony (Conceptual Private User Space / Full Vector Clock):
            </h3>
            <p className="break-all text-sm font-mono text-gray-300 mb-2">
              [{universalModelHarmony28D.slice(0, 7).map(val => val.toFixed(6)).join(', ')}], <br/>
              [{universalModelHarmony28D.slice(7, 14).map(val => val.toFixed(6)).join(', ')}], <br/>
              [{universalModelHarmony28D.slice(14, 21).map(val => val.toFixed(6)).join(', ')}], <br/>
              [{universalModelHarmony28D.slice(21, 28).map(val => val.toFixed(6)).join(', ')}]
            </p>
            <p className="text-gray-400 text-sm mb-2">
              This full 28-dimensional vector represents the **user space of the private network** and acts as a
              **hypergraph binary vector clock** for a specific user interaction. It includes granular details
              (Actor, Action, State, Timestamp) that define a unique, private state within the hypergraph.
            </p>
            <p className="text-gray-400 text-sm font-bold">
              The dimensions 22-28 (the last 7D component, derived from User Transaction Space) are crucial for **protected data sharing**.
            </p>
            <p className="text-gray-400 text-xs">
              When sharing data, the first 21D can be public. However, the information represented by the 22nd to 28th dimensions,
              ...
              can access and interpret the private transactional context and associated data.
            </p>
          </div>
        )}

        {(!universalAddressableModelHarmony14D || !discreetModelLayerVector7D || !universalModelHarmony28D) && (
          <p className="text-gray-400 text-lg">
            Derive all preceding vectors to see the full conceptual roles of 21D and 28D in UBHP's network identity and data protection.
          </p>
        )}
      </div>
      ...
      <div className="mt-16 w-full max-w-4xl bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-center flex flex-col items-center">
        ...
        <div className="mb-6 w-full max-w-xs">
          ...
          >
            <option value="all_inputs">All Quadrant Inputs (5D Rays)</option>
            <option value="derived_vectors">Derived Centroids & Vectors (5D, 6D, 7D, 14D, 21D, 28D)</option>
