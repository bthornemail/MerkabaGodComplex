...
interface QuadrantState {
    ...
    fixedDimensionRay: number[] | null; // New: Fixed-dimension ray for centroid
}
...
const App: React.FC = () => {
    ...
    // Handler for calculating the common centroid
    const handleCalculateCentroid = () => {
        const allFixedRays = quadrantInputs.map(q => q.fixedDimensionRay).filter((ray): ray is number[] => ray !== null);

        if (allFixedRays.length !== 4) {
            setError("Please ensure all four quadrants have valid, processed inputs before calculating the centroid.");
            setCentroidResult(null);
            return;
        }

        // Since createFixedDimensionRay always returns a 5-element array, this check is now mostly for robustness.
        const firstRayDimension = allFixedRays[0].length;
        if (!allFixedRays.every(ray => ray.length === firstRayDimension)) {
            setError("Internal error: Fixed-dimension rays have inconsistent dimensions."); // Should not happen with current logic
            setCentroidResult(null);
            return;
        }
    ...
    return (
        ...
        `}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-6xl mb-8">
                {quadrantInputs.map(quadrant => (
                    <QuadrantInput
                        ...
                        error={error} // Pass error down if needed for specific quadrant errors
                    />
                ))}
            </div>
            ...
            <div className="mt-8 w-full max-w-4xl bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-center">
                <h2 className="text-2xl font-semibold mb-4 text-purple-300">Common Symmetry / Centroid</h2>
                <button
                ...
                >
                    Calculate Centroid of All Quadrants
                </button>

                {centroidResult !== null && (
                    <div className="mt-6 p-4 bg-gray-700 rounded-lg border border-gray-600 text-left">
                        <h3 className="text-lg font-medium text-pink-300 mb-2">Calculated Centroid Vector (5D):</h3>
                        <p className="break-all text-sm font-mono text-gray-300">
                            [{centroidResult.map(val => val.toFixed(6)).join(', ')}]
                        </p>
                        <p className="text-gray-400 text-sm mt-2">
                            This 5-dimensional vector represents the average "harmonic signature" or "common symmetry" of the four inputs.
                            In UBHP, this centroid can signify a point of consensus or a unified understanding derived from multiple domains.
                        </p>
                    </div>
                )}
            </div>
