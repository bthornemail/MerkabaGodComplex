...
const App: React.FC = () => {
    ...
    // New state for semantic model training inputs
    const [sourceTextForEmbeddings, setSourceTextForEmbeddings] = useState<string>('The Universal Binary Hypergraph Protocol redefines digital information as ArrayBuffers, interpreted as Lisp-like S-expressions. This unified binary model underpins a multi-layered, content-addressable hypergraph.');
    const [semanticContextSeed, setSemanticContextSeed] = useState<string>('WordNet, ontology, knowledge graph, semantic network, relationships, definitions, concepts.');
    const [sample7DEmbeddings, setSample7DEmbeddings] = useState<number[][] | null>(null);
    ...
    // Handler for generating sample 7D word embeddings
    const handleGenerateSample7DEmbeddings = useCallback(() => {
        try {
            const encoder = new CanonicalSExprEncoder();
            encoder.encodeString(semanticContextSeed);
            const semanticContextBuffer = encoder.getBuffer();
            const semanticContextSignature = harmonize(semanticContextBuffer);
            const semanticContextRay = createFixedDimensionRay(semanticContextSignature);

            if (!semanticContextRay) {
                setError("Semantic Context Seed could not be processed into a 5D ray.");
                setSample7DEmbeddings(null);
                return;
            }

            const words = sourceTextForEmbeddings.split(/\s+/) // Split by whitespace
                .filter(word => word.length > 0)
                .map(word => word.replace(/[^a-zA-Z0-9]/g, '')) // Remove punctuation
                .filter(word => word.length > 0);

            const numSamples = Math.min(words.length, 5); // Limit to first 5 words for demo
            const generatedEmbeddings: number[][] = [];

            for (let i = 0; i < numSamples; i++) {
                const word = words[i];
                encoder.buffer = []; // Clear encoder buffer for new word
                encoder.encodeString(word);
                const wordBuffer = encoder.getBuffer();
                const wordSignature = harmonize(wordBuffer);
                const wordRay = createFixedDimensionRay(wordSignature);

                if (wordRay) {
                    // Combine wordRay and semanticContextRay into a new 7D vector
                    // Conceptual combination: [word.h, word.sin, word.cos, context.h, context.sin, context.cos, (word.len + context.len)/2]
                    const derived7D: number[] = [
                        wordRay[0], // h from word
                        wordRay[1], // sin from word
                        wordRay[2], // cos from word
                        semanticContextRay[0], // h from semantic context
                        semanticContextRay[1], // sin from semantic context
                        semanticContextRay[2], // cos from semantic context
                        (wordRay[4] + semanticContextRay[4]) / 2 // Average of lengths
                    ];
                    generatedEmbeddings.push(derived7D);
                }
            }
            setSample7DEmbeddings(generatedEmbeddings);
            setError(null);
        } catch (e: any) {
            setError(`Error generating sample 7D embeddings: ${e.message}`);
            setSample7DEmbeddings(null);
        }
    }, [sourceTextForEmbeddings, semanticContextSeed]);
    ...
    return (
        ...
        `}
            ...
            {/* Semantic Model Training Inputs Section */}
            <div className="mt-8 w-full max-w-4xl bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-center">
                <h2 className="text-2xl font-semibold mb-4 text-purple-300">Semantic Model Training Inputs (7D Embeddings)</h2>
                <p className="text-gray-400 text-sm mb-4">
                    Generate conceptual 7D word embeddings by combining individual word harmonies with a semantic context.
                    This demonstrates how UBHP can form training data for an addressed hypergraph.
                </p>

                <div className="mb-4 text-left">
                    <label htmlFor="sourceText" className="block text-gray-300 text-sm font-bold mb-2">
                        Source Text for Embeddings (e.g., Bible, W3C Spec):
                    </label>
                    <textarea
                        id="sourceText"
                        className="w-full p-3 mb-4 rounded-lg bg-gray-700 text-gray-200 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-200 resize-y"
                        rows={6}
                        value={sourceTextForEmbeddings}
                        onChange={(e) => setSourceTextForEmbeddings(e.target.value)}
                        placeholder="Enter text to generate word embeddings from..."
                    ></textarea>

                    <label htmlFor="semanticContext" className="block text-gray-300 text-sm font-bold mb-2">
                        Semantic Context Seed (e.g., WordNet bindings, ontology terms):
                    </label>
                    <textarea
                        id="semanticContext"
                        className="w-full p-3 mb-4 rounded-lg bg-gray-700 text-gray-200 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-200 resize-y"
                        rows={3}
                        value={semanticContextSeed}
                        onChange={(e) => setSemanticContextSeed(e.target.value)}
                        placeholder="Enter semantic context terms..."
                    ></textarea>
                </div>

                <button
                    onClick={handleGenerateSample7DEmbeddings}
                ...
                >
                    Generate Sample 7D Word Embeddings
                </button>

                {sample7DEmbeddings !== null && sample7DEmbeddings.length > 0 && (
                    <div className="mt-6 p-4 bg-gray-700 rounded-lg border border-gray-600 text-left">
                        <h3 className="text-lg font-medium text-pink-300 mb-2">Sample 7D Word Embeddings:</h3>
                        <div className="text-sm font-mono text-gray-300 overflow-x-auto">
                            {sample7DEmbeddings.map((embedding, index) => (
                        ...
                        <p className="text-gray-400 text-xs mt-2">
                            Each 7D vector here represents a word's harmonic signature contextualized by the provided semantic seed,
                            forming a training input for a decentralized hypergraph.
                        </p>
                    </div>
                )}
                {sample7DEmbeddings !== null && sample7DEmbeddings.length === 0 && (
                    <div className="mt-6 p-4 bg-gray-700 rounded-lg border border-gray-600 text-left">
                        <p className="text-gray-400 text-sm">No sample 7D embeddings could be generated. Please check your input texts.</p>
                    </div>
                )}
            </div>
