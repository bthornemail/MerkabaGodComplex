...
const arrayBufferToHexString = (buffer: ArrayBuffer | null): string => {
  if (!buffer) return '';
...
const App: React.FC = () => {
  ...
  // Function to handle encoding and harmonization for an input
  const processInput = (
    input: string,
    ...
    try {
      // Create a new encoder instance for each operation to clear its internal buffer
      const encoder = new CanonicalSExprEncoder();
      encoder.encodeString(input); // Encode the string as an S-expression
  ...
  // Effect to process inputs on initial load or text change
  useEffect(() => {
    processInput(inputText1, setEncodedBuffer1, setHarmonicSignature1);
  }, [inputText1]);

  useEffect(() => {
    processInput(inputText2, setEncodedBuffer2, setHarmonicSignature2);
  }, [inputText2]);
  ...
  const handleCompareHarmonies = () => {
    if (encodedBuffer1 && encodedBuffer2) {
      ...
      setError("Please ensure both inputs have been processed before comparing.");
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 font-sans p-4 sm:p-8 flex flex-col items-center">
      ...
      <div className="flex flex-col lg:flex-row gap-6 w-full max-w-4xl">
        ...
        <div className="flex-1 bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
          <h2 className="text-2xl font-semibold mb-4 text-purple-300">Input 1</h2>
          <textarea
          ...
          ></textarea>
          <button
            onClick={() => processInput(inputText1, setEncodedBuffer1, setHarmonicSignature1)}
        ...
        {/* Input 2 Section */}
        <div className="flex-1 bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700">
          <h2 className="text-2xl font-semibold mb-4 text-purple-300">Input 2</h2>
          <textarea
            className="w-full p-3 mb-4 rounded-lg bg-gray-700 text-gray-200 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-200 resize-y"
            rows={4}
            value={inputText2}
          ...
          ></textarea>
          <button
            onClick={() => processInput(inputText2, setEncodedBuffer2, setHarmonicSignature2)}
            className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500"
          >
            Encode & Harmonize
          </button>
      ...
      <div className="mt-8 w-full max-w-2xl bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 text-center">
        ...
        >
          Compare Harmonies (Cosine Similarity)
        </button>
