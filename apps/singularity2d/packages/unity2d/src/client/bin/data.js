...
import { HostWallet, ProviderWallet, ConsumerWallet,CustomerWallet, QuickWheelWashWallet, ClientWallet, MarketplaceWallet } from './wallets.js'
import { HostAddress, ProviderAddress, ConsumerAddress,CustomerAddress, QuickWheelWashWalletAddress, ClientAddress, MarketplaceAddress } from './wallets.js'
import { wallets, addresses } from './wallets.js'
...
const CleaningProcess = new Map([
	...
	["Finishing Touches","Apply a tire dressing or protectant to enhance the appearance of the tires and provide long-lasting protection against fading and cracking. - Polish the rims with a specialized wheel polish to restore shine and protect against corrosion."]
    ])]
]);
...
const ServiceProviderSchedule = new Map([
    ...
    ["author", QuickWheelWashWalletAddress],
    ["signature", QuickWheelWashWallet.signMessageSync("Quick Wheel Wash/Working Schedule")],
    [startDate, HostAddress],
    [addTime(startDate, 0, 0, 30), ProviderAddress],
    [addTime(startDate, 0, 1), ConsumerAddress],
    [addTime(startDate, 0, 1, 30), null],
    [addTime(startDate, 0, 2), null],
    [addTime(startDate, 0, 2, 30), null],
    [addTime(startDate, 0, 3), null],
    [addTime(startDate, 0, 3, 30), null],
    [addTime(startDate, 0, 4), null],
    [addTime(startDate, 0, 4, 30), null]
]);
...
(async () => {
    if (DatastoreIdb) {
      const store = new DatastoreIdb.IDBDatastore(`${ClientWallet.publicKey}`);
      await store.open();
      await store.put(ClientWallet.publicKey, {
        id: "brian-thorne",
        title: "Brian Thorne",
...
// export { Host , OnlineHost, ServerPort, SocketIOPort, MosquitoPort, ReactPort, RedisPort}
export { Host, HostWallet, Provider, ProviderWallet, Consumer, ConsumerWallet, QuickWheelWashWallet, ClientWallet, MarketplaceWallet }
