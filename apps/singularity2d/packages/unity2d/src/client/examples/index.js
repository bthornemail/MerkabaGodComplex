...
import { bright, red, reset } from '../bin/consoleColors.js'
...
; (async () => {
  ...
  await writeFile('./index.multiaddr', JSON.stringify(node1.getMultiaddrs().map(multiaddr => multiaddr.toString())));//[0].toString()
  await writeFile('./index.peerId', node1.peerId.toString());
  setTimeout(async () => {
    // TASK: switch the commented out line below so we're using a different key, to see the nodes fail to connect 

    console.log('nodes started...')
    const multiaddrs = JSON.parse(await readFile('./xedni.multiaddr', "utf-8")).map(_multiaddr => multiaddr(_multiaddr));
    const _peerID = await readFile('./xedni.peerId', "utf-8");
    ...
    async function send() {
      ...
      return await pipe(
        [uint8ArrayFromString(bright + red + 'This message is sent on a private network ' + reset + process.argv[2])], stream
      )
    }

    return setInterval(async () => await send(), 1500);
  }, 2500)
})()
