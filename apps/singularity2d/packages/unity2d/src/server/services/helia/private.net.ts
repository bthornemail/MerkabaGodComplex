...
import { DefaultIdentifyService } from 'libp2p/dist/src/identify/identify.js'
...
export default async function getPrivateRandomNet (
  options: PRIVATE_NETWORK_INIT_OPTIONS
): Promise<
  Libp2p<{ identify: DefaultIdentifyService, pubsub: PubSub<GossipsubEvents> }>
  > {
  ...
  const peerId: PeerId =
    options.peerId ?? (await createPeerIdFromPrivKey(privateKey))
