...
export default class Token extends BlockNode {
  ...
  async mint(to, amount) {
    ...
    const block = await this.addBlock({ from, to, amount, tokenSignature, timestamp ,pin })
    // console.log(pin);
      ...
      for await (const item of this.node.pins.ls()) {
        if (item.metadata.tokenSignature) {
          ...
          if (verifyMessage(JSON.stringify({ from, to, amount }), item.metadata.tokenSignature) === this.wallet.address) {
            ...
            if (item.metadata.to === address) { items.push(item) }
            if (item.metadata.from === address) { items.push(item) }
          }
        }
      }
      const incomes = items
  ...
  async transfer({ from, to, amount }, signature) {
    ...
    try {
      ...
      // console.log(pin);
      this.node.libp2p.safeDispatchEvent("transfer", { detail: pin })
      this.node.libp2p.safeDispatchEvent("transfer:success", { detail: { from, to, amount, signature, tokenSignature, cid: cid.toString() } })
  ...
  connect = async (node) => {
    ...
    this.node.libp2p.addEventListener('transfer', async (event) => {
      ...
      const { cid, depth, metadata } = event.detail;
    ...
    for await (const item of this.node.pins.ls()) {
      if (item.metadata.tokenSignature) {
        ...
        if (verifyMessage(JSON.stringify({ from, to, amount }), item.metadata.tokenSignature) === this.wallet.address) {
          // console.log(item.metadata);
          // console.log(item.metadata.from,item.metadata.amount,item.metadata.to);
          if (item.metadata.to === this.wallet.address) { this.balances.push(item) }
          if (item.metadata.from === this.wallet.address) { this.balances.push(item) }
        }
      }
    }
    return
