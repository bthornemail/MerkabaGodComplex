...
import HomePage, { MainTopicsButtonGroup} from "./homepage.js"
...
const myWorker = new Worker(new URL('/worker.es.js', import.meta.url), {type: 'module'});
...
function send(message) {
	console.log(viewingChannel, `-+=${wallet}=+-: ${message}`)
	client.publish(`${viewingChannel}/me`, JSON.stringify({content:message}));
}
function addMessage(){
...
function viewChannel(topic) { 
	// store.encode(topic).then((cid) => {
		// store.get(cid).then((data) => {
			...
			// + `<p>${data}</p>`
			messageQueue.filter(([channel])=> channel !== topic).map((p,message)=>{
				alert(p)
				const pg = document.createElement("p");
				pg.innerText = message;
				return p.append(pg)
			})
...
client.on('connect', () => {
	...
	// HomePage(Channel, ConnectWalletButton({getWallet,setWallet}))
	client.subscribe(`${"Marketplace2D"}/#`, (err) => {
		if (err) { console.log("error", err) }
		console.log("Just subscribed to Marketplace2D")
		subscribeToTopic("Connections")
