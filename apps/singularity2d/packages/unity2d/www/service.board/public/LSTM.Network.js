class LSTMNetwork {
    ...
    // LSTM forward pass
    forward(inputs, prevHiddenState, prevCellState) {
    ...
    matrixVectorMultiply(vector, matrix, bias) {
        return matrix.map(row => row.reduce((acc, val, i) => acc + val * vector[i], 0)).map((val, i) => val + bias[i]);
    }
...
function evolvePopulation(population, mutationRate) {
    ...
    console.log(selectedParents[0].exportParameters())
...
// Export parameters
  ...
  let initialPopulation = Array.from({ length: populationSize }, () => new LSTMNetwork(inputSize, hiddenSize, outputSize));
...
const exportedParams = bestIndividual.exportParameters();
console.log("Model:", exportedParams);

// Load parameters (example of loading parameters into a new LSTM instance)
const newLSTM = new LSTMNetwork(inputSize, hiddenSize, outputSize);
newLSTM.loadParameters(exportedParams);
