...
app.post("/api/:topic/message", async (req: Request, res: Response) => {
...
app.get("/api/:topic/message", async (req: Request, res: Response) => {
  console.log(req.params)
  console.log(req.body)
  const { to, from, content } = req.body;
...
app.get("/api/:topic/example", async (req: Request, res: Response) => {
...
// verified
app.post("/api/:author/:topic", async (req: Request, res: Response) => {
  console.log(req.params)
  console.log(req.body)
...
app.get("/api/:author/:topic", async (req: Request, res: Response) => {
  const { author, signature, title, summary, description, content } = await redis.hgetall(req.params.topic)
  if (verifyMessage(JSON.stringify({ title, summary, description }), signature) !== req.params.author) return res.status(200).json({
    topic: req.params.topic,
    message: "Not Verified"
...
app.get("/chat/:from/:to", async (req: Request, res: Response) => {
