...
export async function discoverySocket() {
    ...
    udpServer.bind(3702, () => logger('UDP discovery on 3702'));
    udpServer.on('message', (msg, rinfo) => {
        ...
        quic.send(port, proxy, data)  // Send data to a listening server. `data` is automatically
            ...
            .then(() => { logger("worked") })               // called after the stream is written

            .onError((error: any) => { logger(error) })       // called on error. The error classes for `quic.send` are:
            //   * 'client stream error'
