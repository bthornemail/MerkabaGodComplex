...
import { VAULT_AI_OUTPUT_VALIDATOR } from '../../types/Block.Node.Interfaces.js'
export interface BLOCK_IO_PARAMS {
  input?: Readable;
  output?: Writable;
  blockstore?: Blocks
}
export interface BLOCK_PARAMS {
  name: string;
  description?: string
  inputs?: string[];
  outputs?: string[];
...
export interface BLOCK_DATA_OPTIONS {
  name: string
  description?: string
  inputs?: string[]
  outputs?: string[]
  input?: BLOCK_SCRIPT_FUNCTION_INPUT_ARGUMENT | BLOCK_SCRIPT_FUNCTION_INPUT_ARGUMENT[]
  output?: BLOCK_SCRIPT_FUNCTION_OUTPUT_TYPE | BLOCK_SCRIPT_FUNCTION_OUTPUT_TYPE[]
  validator?: BLOCK_SCRIPT_FUNCTION_OUTPUT_VALIDATOR | BLOCK_SCRIPT_FUNCTION_OUTPUT_VALIDATOR[]
  sourceCode: string
}
...
export interface BLOCK_SCRIPT_FUNCTION_INPUT_ARGUMENT {
  name: string
  value?: null | boolean | object | Array<boolean | object | number | string> | number | string
  type: 'null' | 'boolean' | 'object' | 'array' | 'number' | 'string'
}
export interface BLOCK_SCRIPT_FUNCTION_OUTPUT_VALIDATOR {
  value: string
  type: '==' | '===' | '!=' | '!==' | '>' | '<' | '>=' | '<=' | '&&' | '||' | '!'
}

export interface BLOCK_SCRIPT_FUNCTION_OUTPUT_TYPE {
  value?: string
  asertion?: '==' | '===' | '!=' | '!==' | '>' | '<' | '>=' | '<=' | '&&' | '||' | '!'
  type: 'null' | 'boolean' | 'object' | 'array' | 'number' | 'string'
}
export default class Block extends Transform {
  private dag: DAGCBOR
  name: string
  description?: string
  sourceCode: string
  ...
  data: any[] = []
  assertData: BLOCK_SCRIPT_FUNCTION_OUTPUT_VALIDATOR[] = []

  constructor({ name, description, inputs, outputs, sourceCode }: BLOCK_PARAMS, { blockstore }: BLOCK_IO_PARAMS) {
    super()
    this.name = name
  ...
  async create(blockData: BLOCK_DATA_OPTIONS): Promise<CID> {
    ...
    // this.blockstore = nodeOptions.blockstore ?? new MemoryBlockstore()
    this.name = name
    this.sourceCode = sourceCode
    this.description = description
    // this.input = input
    // this.output = output
    const cid = await this.dag.add(blockData)
  ...
  async load(cid: CID): Promise<BLOCK_DATA_OPTIONS> {
    const {
      name,
      description,
      input,
