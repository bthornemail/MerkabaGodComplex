...
export default function useChat(chatGraphData?: { graphData?: CHAT_GRAPH_DATA, updateGraphData?: any }) {
  ...
  async function sendMessage(message: string, content?: any) {
    ...
    await client?.publishAsync(`chat/${currenTopic}/msgID`, message)
    await client?.publishAsync(`${currenTopic}/chat/${currenTopic}/msgID`, message)
  }
  return {
    isChatting,
    setIsChatting,
