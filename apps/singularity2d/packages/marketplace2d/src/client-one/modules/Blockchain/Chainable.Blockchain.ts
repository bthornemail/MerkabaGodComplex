...
export default class Blockchain<T> {
  ...
  static async createBlock (value: Record<string,any>): Promise<BLOCK_TYPE<Record<string,any>>> {
    // encode a block
    return await Block.encode<Record<string,any>,512,18>({ value: value, codec, hasher })
  }
  ...
  validateBlock (block: BLOCK_TYPE<T>): boolean {
    ...
    console.log( block, this.chain[0])
