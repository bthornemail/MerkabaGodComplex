    ...
    return (<div className="input-group" style={{ minWidth: "fit-content" }}>
        ...
        <div id="connections-bar" className="input-group" style={{ minWidth: "fit-content", border: "rgba(0,0,0,0.55)" }}>
            {Object.entries(layers).map(([protocol, attributes], index) => {
                return <Link key={index} to={protocol}><button key={index} className={`btn btn-sm btn-outline-${Object.entries(colors)[index % 6 ][0] || "light"} rounded-3  me-2`} onClick={() => console.log(protocol)}>{attributes.details.title}</button></Link>
            })}
        </div></>
        </div>
    </div>)
