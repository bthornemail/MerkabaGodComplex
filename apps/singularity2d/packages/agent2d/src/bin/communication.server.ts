...
export async function communicationServer({ challenges, mqttClient, publicEndpoints, sseClients, udpServer, withNgrok }: {
    ...
    const rootFolder = 'public/src/docs'; // Update with your folder path
    ...
    wsServer.on('connection', (ws) => {
        ...
        ws.on('message', (data) => {
            ...
            log(`Message from ${peerId}:`, message.toString());
            // switch (message.type) {
    ...
    // Start SSE server
    sseServer.listen(3001, () => log('SSE server on http://localhost:3001'));
    ...
    async function* fileStream() {
        let previousState = "";
        while (true) {
            const peerList = JSON.stringify(Array.from(files.keys()));
    ...
    watcher
        .on('add', (path: any) => {
            ...
            files.set(path,file);
            log(`File added: ${path}`);
            // log(`File added: ${path}`, file.split("\n"));
        })
        .on('change', (path: any) => {
            const file = readFileSync(path, "utf8");
            files.set(path,file);
            log(`File changed: ${path.split("/")}`);
            // log(`File changed: ${path.split("/")}`, file.split("\n"));
        })
        .on('unlink', (path: any) => {
            files.delete(path);
    ...
    log(`Watching for changes in: ${rootFolder}`);
    ...
    for await (const update of fileStream()) {
        mqttClient.publishAsync("docs",update);
