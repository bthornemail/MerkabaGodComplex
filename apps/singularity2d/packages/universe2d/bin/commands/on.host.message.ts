...
export default async function onHostMessage(link: string, message: string, signer: HDNodeWallet): Promise<[string, string]> {
    ...
    // }, message.toString())
    try {
        const decryptedString = await decryptString(message, [
            host,
            provider,
            consumer,
            service,
        ...
        return [url.href, decryptedString];
    } catch (error) {
