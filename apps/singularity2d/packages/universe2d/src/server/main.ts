...
app.use(cors({
	origin: ["*",ngrokURL,"http://localhost:5173"], // ngrokURL, // "http://localhost:5173", // Allow requests from your frontend
	methods: ["POST", "GET"],
...
app.post("/upload", express.json(), async (req, res) => {
	...
	const transcription = await transcribeBuffer(Buffer.from(req.body.audio));
...
app.post("/chat", express.json(), async (req, res) => {
	...
	console.log({ content });
	res.json({ type: "message", role: "assistant", content });
});
