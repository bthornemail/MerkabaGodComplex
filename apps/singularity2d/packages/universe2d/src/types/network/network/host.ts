...
import Graph, { GRAPH_DATA } from "../storage/graph";
export type HOST = {
    extendedKey: string;
    graph: GRAPH_DATA;
};
export interface iHost {
    node: Node;
    graph: Graph;
    post(key: string, context: CONTEXT, signature?: string): Promise<string>;
    pull(key: string, hash: string): AsyncGenerator<CONTEXT,string,void>
    get: (key: string) => Promise<CONTEXT>;
};
abstract class BaseHost implements iHost{
    protected signer: HDNodeWallet;
    node: Node;
    graph: Graph;
    abstract post(key: string, context: CONTEXT, signature?: string): Promise<string>;
...
export default class Host extends BaseHost {
    ...
    constructor(key: Identity,node: NODE) {
        ...
        const chain = this.graph = new Graph({ extendedKey: signer.extendedKey });
    }
