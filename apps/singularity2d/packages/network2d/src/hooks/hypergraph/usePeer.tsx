...
export const PeerContext = createContext<any>({
    wallet: null, peerId: null,
    generate: null, encrypt: null, decrypt: null, unlock: null, update: null,
    auto: null, register: null, authenticate: null, create: null,
})
export const PeerProvider = ({ children }: any) => {
    const context = useContext(PeerContext)
    return <PeerContext.Provider value={context}>{children}</PeerContext.Provider>
}
...
export const usePeer = () => {
    ...
    const [wallet, setWallet] = useState<HDNodeWallet | HDNodeVoidWallet>();
    ...
    function unlock({ key, extendedKey, signature }: any) {
        if (extendedKey) {
            setWallet(HDNodeWallet.fromExtendedKey(extendedKey) as any)
            return;
        }
        if (key && signature) {
    ...
    const auto = () => {
        ...
        navigator.credentials
            .create(createCredentialDefaultArgs as any)
            .then((cred) => {
                ...
                const idList = [
                    {
                        id: cred?.["rawId"] ? cred["rawId"]! : cred?.id,
    ...
    return {
        ...
        auto, register, authenticate, create,
    }
}
